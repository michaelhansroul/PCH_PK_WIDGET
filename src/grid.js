...
// =============================
/*
esribelux.core.onLoad(function()
{
	var itemProp = {
		'data': [
			{'OBJECTID':1, 'ID':'MHN', 'NAME':'Michael Hansroul'},
			{'OBJECTID':2, 'ID':'SU', 'NAME':'Simon Uyttendaele'},
			{'OBJECTID':3, 'ID':'DMT', 'NAME':'David Mutibul'},
			{'OBJECTID':4, 'ID':'ER', 'NAME':'Emile Robe'},
			{'OBJECTID':5, 'ID':'GEG', 'NAME':'Geraldine Eggermont'},
			{'OBJECTID':6, 'ID':'SDC', 'NAME':'Stephanie de Calatay'},
			{'OBJECTID':7, 'ID':'LDC', 'NAME':'Ludovic Dalla Corte'},
			{'OBJECTID':8, 'ID':'VSW', 'NAME':'Vincent Swerts'}
		],
		'columns':[
			{'field':'OBJECTID', 	'label':'OBJECTID', 	'isEditable': false, 	'isVisible': false, 	'isSortable': false, 	'type':'esri...'},
			{'field':'NAME', 		'label':'Name', 		'isEditable': true, 	'isVisible': true, 		'isSortable': true, 	'type':'esri...'},
			{'field':'ID', 			'label':'Initials', 	'isEditable': false, 	'isVisible': true, 		'isSortable': true, 	'type':'esri...'}
		],
		'isSelectable': true,
		'isAppendable': true,
		'isRemovable': true,
	...
	new esribelux.Grid('bottompane', itemProp);
});*/

esribelux.Grid = function(div, properties)
{
	var self = this;
	this.dom_table = null;
	...
	this.initialize_header = function()
	{
		...
		if( this.isSelectable )
		{
			...
			dojo.connect(checkbox, 'onclick', function() { self.toggleSelectionOnly(); });
			tr.appendChild(th);
		}
	...
	this.initialize_footer = function()
	{
		...
		dojo.connect(td, 'onclick', function() { self.add(); });
		tr.appendChild(td);
	...
	this.initialize_pager = function()
	{
		...
		dojo.connect(previous, 'onmousedown', function() { self._progressiveScrollPageTimeout(500, -1); });
		dojo.connect(previous, 'onmouseup', function() { if( self.__handler_pspt ) { clearTimeout(self.__handler_pspt); self.__handler_pspt = null; } });
		dojo.connect(previous, 'onclick', function() { if( self.__current_page > 0 ) self.paging(self.__current_page -1); });
		
		dojo.connect(next, 'onmousedown', function() { self._progressiveScrollPageTimeout(500, 1); });
		dojo.connect(next, 'onmouseup', function() { if( self.__handler_pspt ) { clearTimeout(self.__handler_pspt); self.__handler_pspt = null; } });
		dojo.connect(next, 'onclick', function() { if( self.__current_page < self.getMaxPages() ) self.paging(self.__current_page +1); });
		
		dojo.connect(this, 'onPaging', function(page)
		{
			var max = self.getMaxPages();
			number.innerHTML = (page+1) + " / " + (max+1);
			if( page <= 0 )
				addClassName(previous, 'disabled');
			else
				removeClassName(previous, 'disabled');
	...
	this._append_row = function(index)
	{
		...
		if( this.isSelectable )
		{
			...
			dojo.connect(zoom, 'onclick', function() { self.onZoom(self.getRow(tr)); });
			td.appendChild(zoom);
	...
	this._append_edit_icon = function(tr, td)
	{
		...
		dojo.connect(edit, 'onclick', function() { self.onEditRow(self.getRow(tr), tr); });
		td.appendChild(edit);
	}
	...
	this._allow_editing = function(td)
	{
		dojo.connectOnce(td, 'ondblclick', function()
		{
			...
			input.type = 'text';
			input.value = value;
			input.style.width = (td.clientWidth - 10 /* padding */) + 'px';
	...
	this._allow_sort = function(th)
	{
		...
		th.title = 'Sort rows';
		dojo.connect(th, 'onclick', function()
		{
	...
	this.paging = function(page)
	{
		...
		for( var i = 0; i < this.rowsPerPage; i++ )
		{
			if(this.isSelectable)
			{
				removeClassName(this.dom_body.rows[i], 'selected');
			...
			for( var i = 0; i < this.data.length; i++ )
			{
				if( this.data[i] === this.selection[j] )
				{
					...
					if( k >= 0 )
					{
						...
						{
							addClassName(this.dom_body.rows[k], 'selected');
	...
	this.sort = function(field)
	{
		...
		{
			removeClassName(th[i], 'asc');
			removeClassName(th[i], 'desc');
			if( th[i].innerHTML == column.label )
				addClassName(th[i], (this._sortOrder ? 'asc' : 'desc'));
		}
	...
	this.select = function(row)
	{
		...
		{
			esribelux.status.log("GRID SELECT", esribelux.status.WARNING);
		...
		for( var i = 0; i < this.data.length; i++ )
		{
			if( this.data[i] === row )
			{
				...
				if( k >= 0 )
				{
					if(this.dom_body.childNodes.length>k)
					{
						removeClassName(this.dom_body.childNodes[k], 'selected');
	...
	this.showSelectionOnly = function(value)
	{
		...
		if( value )
			addClassName(this.dom_table, 'selectionOnly');
		else
			removeClassName(this.dom_table, 'selectionOnly');
